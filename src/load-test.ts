...
async function runCreateUsers(): Promise<any[]> {
  const prom = userIds.map((userID, i) =>
    createUsers(
      userID,
      `${(Math.random() + 1).toString(36).substring(7)}+${i}@gmail.com`
    )
  );
...
async function purchase(userId: string): Promise<any> {
  try {
    ...
    return { user_id: userId, success: true, data: response.data.data };
  } catch (error: any) {
    return {
      user_id: userId,
      success: false,
...
async function runLoadTest() {
  ...
  for (const batch of batches) {
    ...
    const filteredResults = results.filter((result) => result.success);

    const total = filteredResults.reduce((total, result) => {
      const quantity = result?.data?.quantity || 0;
      console.log(`Processing result quantity`, quantity);
      return total + quantity;
    }, 0);

    totalItemsPurchased += total;
    // Small delay between batches to simulate realistic user behavior
    await new Promise((resolve) => setTimeout(resolve, 100));
  }

  console.log(
    `Test completed: ${successCount} successful purchases, ${failCount} failed purchases`
  );

  // Verify final inventory
  try {
    const response = await axios.get(
      `${BASE_URL}/flash-sales/67c6229af3aacb324d59745b`
    ...
    if (
      ...
      response.data.data.currentInventory == 0
    ) {
      console.log(
        "✅ Test PASSED: Exact inventory sold, no overbuying or underbuying"
      );
      ...
      console.log(
        "❌ Test FAILED: Underbuying detected - not all inventory was sold"
      );
    } else if (
      totalItemsPurchased < 200 &&
      response.data.data.currentInventory > 0
...
async function verifyFlashSaleActive() {
  try {
    ...
    if (response.data.data.status !== "active") {
      console.log(
        "Flash sale is not active. Please start the flash sale first."
      );
      process.exit(1);
    }
